...
{
  constexpr std::size_t operator"" _z(unsigned long long n) { return std::size_t{n}; }
  ...
  constexpr void apply(const It it, const CAS cas_f, const cas_node<A, B>)
  {
    detail::iterator_compare_and_swap<It, CAS>()(it + A, it + B, cas_f);
  }

  template<typename It, typename CAS, typename... LayerOrCas>
  constexpr void apply(const It it, const CAS cas, const layer<LayerOrCas...>)
  {
    (apply(it, cas, LayerOrCas{}), ...);
  }

  template<typename It, typename CAS, typename... Layers>
  constexpr void apply(const It it, const CAS cas, const net<Layers...>)
  {
  ...
  namespace detail
  {
    ...
    struct iterator_compare_and_swap
    {
      constexpr void operator()(const It a, const It b, const CAS compare_and_swap) const { compare_and_swap(*a, *b); }
    };

    constexpr bool is_power_of_two(const std::size_t n) { return (n > 0) && !(n & (n - 1)); }
  ...
  struct available<N, Network, std::void_t<decltype(detail::sorting_network<N, Network>{})>>
    : std::true_type
  {};
